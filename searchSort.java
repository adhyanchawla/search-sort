...
public class searchSort {
    ...
    public int minEatingSpeed(int[] piles, int h) {
        if(piles.length == 1) {
            return (piles[0] % h != 0) ? piles[0] / h + 1 : piles[0] / h;    
        }
    ...
    public int findRadius(int[] houses, int[] heaters) {
        ...
        int rad = 0;
        for(int ele : houses) {
            int f = floor(heaters, ele);
            int c = ceil(heaters, ele);
            f = (f == -1) ? Integer.MAX_VALUE : f;
            c = (c == Integer.MAX_VALUE) ? Integer.MAX_VALUE : c;
            int fdiff = (int)Math.abs(f - ele);
            int cdiff = (int)Math.abs(c - ele);
            int max = Math.min(fdiff, cdiff);
            // System.out.println(max);
            rad = Math.max(max, rad);
        }
        return rad;
    }
